module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  return api.sendMessage("Admin bot mới dùng bot trong đoạn chat riêng!!", threadID, messageID)
    ...
    if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {
     ...
     const dataAdbox = require('./../../Data_Vtuan/data/data.json');
  ...
  /*   const fileSystem = require("fs").promises;
     ...
     let currentData = [];
         const fileContent = await fileSystem.readFile(rentbotPath, { encoding: 'utf-8' });
         currentData = JSON.parse(fileContent);

         const isAdmin = config.ADMINBOT.includes(event.senderID);
         const isNDH = config.NDH.includes(event.senderID);
         let isThreadIdInData = false;
         let ThueBotValue = 1;

         const rentbotEntry = currentData.find(entry => entry.id1 == event.threadID || entry.id2 == event.threadID);
         if (rentbotEntry) {
             isThreadIdInData = true;
             ThueBotValue = rentbotEntry.ThueBot;
         }
       
console.log(ThueBotValue)
       if (ThueBotValue == 2) {
            return api.sendMessage(`Nhóm của bạn chưa thuê bot nên không xử dụng được lệnh này!!\nLiên hệ admin qua fb: ${(!global.config.FACEBOOK_ADMIN) ?  "Thêm facebook admin ở config!" : global.config.FACEBOOK_ADMIN}`, event.threadID, event.messageID);
        } */
    ...
    if (command.config.hasPermssion > permssion) return api.sendMessage(`Quyền hạn của lệnh: ${command.config.name} là ${quyenhan}`, event.threadID, event.messageID);
       ...
       if (!client.cooldowns.has(command.config.name)) client.cooldowns.set(command.config.name, new Map());
        ...
        if (timestamps.has(senderID) && dateNow < timestamps.get(senderID) + expirationTime) 
          return api.sendMessage(`== [ Cảnh Báo Spam ] ==\n‣ ${time}\n‣ Bạn có thể bị cấm sử dụng bot nếu cố tình vi phạm!\n‣ Vui lòng thử lại sau ${((timestamps.get(senderID) + expirationTime - dateNow)/1000).toString().slice(0, 5)}s`, threadID,(err, info)  => setTimeout ( () => { api.unsendMessage(info.messageID) } , 5000));
