...
const path = pathModule.join(__dirname, '../../Data_Vtuan/data/shortcutdata.json');
const pathGif = pathModule.join(__dirname, '../../Data_Vtuan/data/shortcut');
...
module.exports.handleEvent = async function ({ event, api, Users }) {
  ...
  if (data.some(item => item.input == body)) {
    ...
    const path = pathModule.join(__dirname, `../../Data_Vtuan/data/shortcut/${dataThread.id}`);
    ...
    case "requireGif": {
      ...
      if ((event.attachments || []).length > 0) {
        try {
          ...
          const pathGif = pathModule.join(__dirname, '../../Data_Vtuan/data/shortcut', id);
    ...
    case "-a": {
      ...
      if (data.length == 0) return api.sendMessage("[ Shortcut ] - hiện tại nhóm của bạn chưa có shortcut nào được set!", threadID, messageID);
        ...
        for (const single of data) {
          const path = pathModule.join(__dirname, `../../Data_Vtuan/data/shortcut/${single.id}`);
          var existPath = false;
